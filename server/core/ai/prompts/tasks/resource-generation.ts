...
function createResourcePrompt(spec: ResourcePromptSpec): PromptContract<ResourceGenerationInput> {
  const contract = {
    ...
    process: [
      ...
      '画像上下文若包含“生成前语义学习裁决”，必须复用其中的规范概念和掌握状态：已掌握内容不得从头重讲，只能作为应用、边界、对比或迁移的前置知识。',
      '语义裁决给出类比桥梁时，明确写出相同机制与关键差异，利用已学概念降低理解成本，但不能把两个概念合并成同一概念。',
    ...
    incorrect: [
      ...
      '重新讲授语义裁决中已经掌握的基础内容，或者把相似机制误写成同义概念。',
      ...spec.incorrect,
    ],
  };
...
export const RESOURCE_GENERATION_PROMPTS: Record<ResourceGenerationType, PromptContract<ResourceGenerationInput>> = {
  ...
  video: createResourcePrompt({
    ...
    extraStandards: [JSON_OUTPUT_STANDARD],
    process: [
      '先写教学分镜：问题钩子 → 核心机制 → 例子或过程 → 边界/误区 → 总结与行动。',
      '用 4 到 6 个场景表达一条连续因果链，每个场景只承载一个主要教学意图。',
      '每幕必须让用户知道“正在讲什么、为什么重要、应该看哪里”，不能只摆装饰图形。',
      '标题和字幕负责叙事，elements 负责视觉证明；不要把整段讲稿塞进一个文本框。',
    ],
    output: [
      '用 ```json 代码块包裹严格 JSON。',
      'JSON 顶层包含 title、scenes、width=1920、height=1080、fps=24。',
      'scenes 为 4 到 6 个场景；每幕必须包含 id、duration、eyebrow、title、subtitle、narration、accentColor、backgroundColor、elements。',
      'title 为 8-24 字核心结论；subtitle 为 18-60 字解释；narration 为 25-90 字口播字幕，必须包含有信息量的介绍文字。',
      '每幕包含 2 到 5 个 text/code/shape 元素；正文区域 y 建议从 390 开始，底部 160 像素留给 narration。',
      'text 元素应包含 role(body/caption/badge)、width、height、fontSize、animation；code 元素必须给 language 和可运行或可推理的短代码。',
      'animation 从 slideIn、scale、reveal、float、draw、fadeIn 中按元素含义选择，并用 delay 做 0.12-0.25 秒错峰入场；禁止所有元素同时出现。',
      '至少两幕使用结构关系、代码、对比或过程图形来证明结论，不能每幕都只是文字列表。',
      '总时长控制在 30 到 60 秒；每幕 5 到 10 秒。',
    ],
    correct: [
      'JSON 能被解析并渲染。',
      '场景顺序表达清楚的教学推进，开头有问题、过程有解释、结尾有可执行总结。',
      '每幕都有标题、介绍文字、视觉证据和字幕，画面层次明确。',
    ],
    incorrect: [
      '缺少 scenes、width、height 或 fps。',
      '场景过少、没有介绍文字、只有装饰元素或内容无法教学。',
    ],
  }),
