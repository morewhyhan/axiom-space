...
export default function BottomBar() {
  ...
  const cumul = growth?.map(g => g.cumulative) || []
  ...
  const areaDaily = dailyPath + ` L${daily.length > 0 ? (daily.length - 1) * 100 : 600},85 L0,85 Z`
  const areaCumul = cumulPath + ` L${cumul.length > 0 ? (cumul.length - 1) * 100 : 600},85 L0,85 Z`
      ...
      <div className="flex items-stretch gap-6" style={{ height: 'var(--bottom-h)' }}>
        ...
        <div className="flex-1 relative min-w-0 flex flex-col">
          ...
          <div className="flex justify-between opacity-25 uppercase tracking-widest px-1 border-t border-white/5 pt-1" style={{ fontSize: 'var(--f8)' }}>
            ...
            <span>{loading ? '…' : daily.reduce((s, v) => s + v, 0)} CARDS</span>
          </div>
        </div>
        ...
        <div className="flex-[2] relative min-w-0 flex flex-col">
          <svg viewBox="0 0 600 100" preserveAspectRatio="none" className="flex-1 w-full">
            ...
            <path d={areaCumul} fill="url(#spPurple)" opacity="0.4" />
            <path d={cumulPath} fill="none" stroke="#a855f7" strokeWidth="1.5" style={{ filter: 'drop-shadow(0 0 5px #a855f7)' }} />
          </svg>
          <div className="flex justify-between uppercase tracking-widest px-1 border-t border-white/5 pt-1" style={{ fontSize: 'var(--f8)' }}>
            <span className="opacity-25">7 DAY CUMULATIVE</span>
            <span className="text-purple-400/70 font-bold">{loading ? '…' : stats?.totalNodes ?? 0} TOTAL</span>
          </div>
        </div>
