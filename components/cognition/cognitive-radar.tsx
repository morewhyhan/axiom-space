    ...
    <aside className="side-slot visible cognition-panel flex-col pointer-events-auto gap-4" style={{ width: 'var(--panel-sm)' }}>
      <div className="glass-panel p-5 rounded-2xl">
        ...
        <div className="flex justify-center mb-4">
          <svg width="180" height="180" viewBox="0 0 200 200">
            ...
            {dimLabels.map((label, i) => {
              ...
              return <text key={label} x={lx} y={ly} textAnchor={anchor} fill="rgba(255,255,255,0.35)" fontSize="7" fontFamily="JetBrains Mono">{label}</text>
            })}
          </svg>
        </div>
        ...
        <div className="space-y-2">
          {[
            ...
            <div key={d.label}>
              <div className="flex justify-between mono mb-0.5" style={{ fontSize: 'var(--f8)' }}>
                <span className="opacity-35">{d.label}</span>
      ...
      <div className="glass-panel p-4 rounded-2xl">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="text-center"><span className="serif font-bold text-purple-400" style={{ fontSize: 'var(--t-title)' }}>{loading ? '—' : stats.streakDays}</span><span className="mono opacity-25 block" style={{ fontSize: 'var(--f7)' }}>天连续</span></div>
          <div className="text-center"><span className="serif font-bold text-cyan-400" style={{ fontSize: 'var(--t-title)' }}>{loading ? '—' : stats.mastered}</span><span className="mono opacity-25 block" style={{ fontSize: 'var(--f7)' }}>已掌握</span></div>
          <div className="text-center"><span className="serif font-bold text-pink-400" style={{ fontSize: 'var(--t-title)' }}>{loading ? '—' : stats.pendingReview}</span><span className="mono opacity-25 block" style={{ fontSize: 'var(--f7)' }}>待复习</span></div>
          <div className="text-center"><span className="serif font-bold text-white/50" style={{ fontSize: 'var(--t-title)' }}>{loading ? '—' : stats.chatRounds}</span><span className="mono opacity-25 block" style={{ fontSize: 'var(--f7)' }}>对话轮次</span></div>
        </div>
      </div>
      ...
      <div className="glass-panel p-4 rounded-2xl">
        ...
        <div className="flex flex-wrap gap-1.5">
          {loading ? (
            ...
            skills.map(skill => (
              <span key={skill.name} className={`px-2 py-1 ${skill.level === 'active' ? 'bg-purple-500/10 text-purple-300/70' : 'bg-white/5 text-white/40'} mono rounded`} style={{ fontSize: 'var(--f8)' }}>
                {skill.name}
              </span>
            ))
          ) : (
