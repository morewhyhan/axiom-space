# AXIOM Space 系统设计与开发说明书

> 文档编号：03  
> 文档性质：系统设计与开发实现说明  
> 核心职责：回答“AXIOM Space 如何把需求变成可以运行、可以追溯的多智能体学习系统？”  
> 上游文档：[02-软件需求规格说明书](./02-软件需求规格说明书.md)、[10-产品创新与设计决策全记录](./10-产品创新与设计决策全记录.md)  
> 下游文档：[04-测试说明书与测试报告](./04-测试说明书与测试报告.md)、[05-部署说明书](./05-部署说明书.md)

## 技术方案总览

**AXIOM Space 的技术创新不在于“接入了一个大模型”，而在于用 Agent Harness 把模型的发散能力牵引到同一个可验证学习对象上。**

~~~text
课程资料 / 学习主题
  → 动态画像识别“这个学生是谁、目前卡在哪里”
  → Learn 把目标组织成可执行路径
  → Forge 将对话绑定具体卡片和学习步骤
  → Agent A 负责教学与追问
  → Agent B 负责观察、画像、卡片和证据
  → 子 Agent 按需完成审核、资源、评估等隔离任务
  → 用户自己的输出进入卡片
  → 审核与评估区分“写清楚”和“真正掌握”
  → RAG、图谱、路径、画像和推送共同决定下一步
~~~

| 技术问题 | AXIOM Space 的设计决定 | 对学生的直接价值 |
|---|---|---|
| 通用 Agent 输出停在回复 | 自研 Harness 绑定入口、上下文、角色、工具、证据、反馈和知识 | 一次对话能改变卡片、画像、路径、资源和图谱 |
| 多 Agent 容易变角色表演 | 双 Agent 底座负责前台教学与后台观察，子 Agent 按需隔离 | 教学、记录和审核彼此制约，过程可追踪 |
| 画像容易变静态标签 | 来源、置信度、历史快照和教学策略编译 | 画像会改变下一问、资源、路径或推送 |
| 向量库容易成为第二事实源 | Postgres 保存事实，RAG 只是可重建派生索引 | 召回旧知识但不污染真实卡片 |
| 图谱容易成为 3D 装饰 | Card / Edge / Cluster 契约与多种认知投影 | 同一证据可从结构、任务、来源和掌握角度查看 |
| AI 生成容易伪造掌握 | 用户输出、卡片准入、独立审核、迁移评估分层 | “AI 说过”不会被当成“学生会了” |
| 多模态生成耗时且脆弱 | 状态机、进度事件、失败隔离、类型校验和预览 | 用户知道系统在做什么，部分失败不吞掉成功结果 |

## 目录

- [1. 设计目标与开发原则](#1-设计目标与开发原则)
- [2. 总体架构](#2-总体架构)
- [3. 核心运行闭环](#3-核心运行闭环)
- [4. 多智能体与 Agent Harness](#4-多智能体与-agent-harness)
- [5. 领域对象与数据事实](#5-领域对象与数据事实)
- [6. 动态画像与教学策略](#6-动态画像与教学策略)
- [7. 路径、资源、推送与评估](#7-路径资源推送与评估)
- [8. RAG 与知识图谱](#8-rag-与知识图谱)
- [9. UI、系统集成与工程实现](#9-ui系统集成与工程实现)
- [10. 可信、安全、性能与边界](#10-可信安全性能与边界)
- [11. 需求到实现的追溯](#11-需求到实现的追溯)
- [12. 对象真源与设计决策索引](#12-对象真源与设计决策索引)

## 1. 设计目标与开发原则

### 1.1 设计目标

系统设计围绕一个可否证目标展开：学生完成一次学习后，系统不仅保存 AI 的回答，还要保存学生自己的输出、为什么通过或未通过、下一步如何变化，以及下次如何继续。
...
1. **依赖从外指向内。** 核心业务不依赖 Next.js、Hono、Prisma 或具体模型 SDK。
2. **数据库是事实源。** 缓存、RAG、向量索引和页面读模型都是可重建派生层。
3. **所有 AI 动作落到业务对象。** 回复本身不是业务完成。
4. **错误显式暴露。** 部分成功、失败、重试和降级具有不同状态。
5. **用户输出优先。** AI 不把自己的内容冒充成用户掌握。
6. **清晰、准确、必要。** 长期知识必须对象清楚、有依据、通过删除测试。
7. **类型与契约统一。** 前后端通过 Hono RPC 和共享领域类型减少漂移。
8. **页面展示证据。** 架构能力最终必须变成用户可见的状态、理由和结果。
...
| 层次 | 技术与职责 |
|---|---|
| 表现层 | Next.js App Router、React、Tailwind CSS、shadcn/ui |
| 状态与数据请求 | React Query 管理服务端数据，Zustand 管理全局 UI 状态 |
| API 网关 | Hono 路由、参数验证、统一错误、RPC 类型推导 |
| 核心业务 | 纯 TypeScript 领域对象、Agent、画像、学习、RAG、推送、评估 |
| 基础设施 | Prisma、PostgreSQL / SQLite、文件存储、模型与 RAG 适配器 |
| 认证 | Better Auth |
| 异步与事件 | 后台任务、通知总线、资源进度事件、状态查询 |
| AI 与检索 | DeepSeek / OpenAI-compatible 模型调用、讯飞星火 WebSocket 适配、Agent Runtime、Qdrant 语义索引与 LightRAG 图增强检索 |
...
server/api
  ↓ 校验、鉴权、错误转换、转发
server/core
  ↓ 领域对象、业务规则、Agent、学习、评估、RAG 契约
server/infra
  ↓ 数据库、文件系统、模型、向量库、外部服务适配
~~~

核心法则是：内层不认识外层。页面不能直接承担领域规则；API 不能把业务逻辑堆在路由中；核心域不能依赖框架实现。
...
课程资料
  → 文档解析
  → 概念与来源提取
  → literature / fleeting 卡片
  → WikiLink / Edge / Cluster
  → 学习路径
  → RAG 索引任务
~~~

资料导入不是收藏。它必须把外部内容变成可学习对象，并保留失败边界：某个后续步骤失败时，已经合法写入的卡片不能被无提示抹掉。
...
LearningPathStep
  → 绑定或创建 Card
  → 创建或恢复 LearningSession
  → 构建 Current Focus
  → 装配画像、旧知识、路径、资源和证据上下文
  → Agent A 开始教学
~~~
...
## 4. 多智能体与 Agent Harness

### 4.1 Harness 的工作定义

Harness 不是一个大而全的 Agent 类，而是七类牵引机制的组合：

1. **入口：** 当前 Vault、课程、路径、步骤和卡片。
2. **上下文：** 用户画像、对话、旧卡、检索引用、资源和评估。
3. **角色：** Agent A、Agent B 和按需子 Agent。
4. **工具：** 有类型、有风险等级、有确认边界的业务能力。
5. **证据：** 来源、用户原话、对象标识、评估和审计记录。
6. **反馈：** 页面事件、进度、失败、建议和用户确认。
7. **知识：** 数据库事实、个人卡片、课程资料和派生索引。

模型仍可以生成多样输出，但 Harness 决定它看到什么、能做什么、何时写入、写入哪里、如何被验证。

### 4.2 双 Agent 底座

- **Agent A：前台教学 Agent。** 负责围绕当前学习对象解释、提问、追问、提示和组织用户输出。
- **Agent B：后台分析 Agent。** 负责从真实对话中提取观察、更新画像候选、同步卡片线索和生成后续干预建议。

双 Agent 的价值不是“两个聊天头像”，而是将教学和记录分开：正在说服用户的 Agent 不应同时不受约束地证明自己教学成功。
...
子 Agent 不需要全部常驻，也不需要在页面上被包装成固定角色。它们的价值是隔离目标、上下文、工具和失败。

### 4.4 工具治理

工具是模型影响系统的唯一受控边界。每项工具需要定义：
...
Agent Harness 通过统一模型配置与适配层隔离供应商差异。当前运行基线采用 DeepSeek 的 OpenAI-compatible 接口；科大讯飞星火已完成 WebSocket 工程接入，可按部署方案启用。

讯飞适配层已经实现：

- v1.1、v2、v3.5 三个协议版本；
- 基于 HMAC-SHA256 的服务端鉴权；
- 统一消息到星火请求结构的转换；
- 分片聚合、流式回调和 Token 用量读取；
- API 错误、连接错误与超时控制；
- 登录保护的配置检查与在线兼容性探测接口。

模型供应商切换不改变 Card、Path、Profile、Assessment 等领域对象，也不绕过工具 schema、用户确认、Vault 隔离和审计机制。完整的 AI 工具与研发流程披露见 [09-AI 工具使用与 AI 辅助开发流程说明](./09-AI工具使用与AI辅助开发流程说明.md)。
...
- Vault：个人知识与学习边界。
- Card：知识对象；区分 fleeting、literature、permanent。
- LearningPath：学习任务组。
- LearningPathStep：可执行任务。
- LearningSession / LearningMessage：围绕对象的学习过程。
- EducationProfile / Observation / ProfileSnapshot：画像事实、来源和历史。
- Assessment / Mastery / Evidence：学习结果与依据。
- Resource / PushRecord：学习资源与推送决策。
- RagDocumentIndex / RagReference：派生索引与真实引用。
- Cluster / Edge / WikiLink：知识结构和关系。
- BackgroundJob / Notification / AuditLog：异步执行、页面反馈和可追踪性。

### 5.2 三类卡片

| 类型 | 含义 | 主要来源 | 是否代表掌握 |
|---|---|---|---|
| fleeting | 零散想法、疑问、尚未完成的理解 | 用户表达、学习过程 | 否 |
| literature | 课程、文献、AI 资源或外部资料 | 外部来源 | 否 |
| permanent | 通过长期知识准入审核的用户理解 | 用户输出与审核 | 只代表内容准入，不自动代表全部能力掌握 |

### 5.3 事实与派生层

- Prisma 数据库保存用户、卡片、路径、会话、画像、评估和资源事实。
- RAG、向量索引、隐藏关系和页面统计是可重建派生数据。
- 派生层可以提供建议，不能反向篡改事实对象。
- 索引失败与事实保存失败必须是两个不同状态。

## 6. 动态画像与教学策略

### 6.1 画像不是六个分数

画像至少包含知识基础、目标、表达或认知偏好、易错点、节奏、资源偏好等维度，但每个维度还应包含：
...
- **即时轻量通道：** 从当前对话形成候选观察，为下一问提供低成本调整。
- **后台深度通道：** 汇总多轮消息、卡片、评估和行为，生成更稳定的画像更新。

### 6.3 从画像到教学行动

画像不能停在 Cognition 展示页。它需要被压缩为教学策略，例如：
...
路径由目标、前置关系、当前画像、已有卡片和掌握证据共同决定。路径调整需要保留原因，避免用户只看到顺序变化却不知道为什么。

### 7.2 资源

资源编排从“当前缺口和用途”开始，而不是从“生成一个文件”开始。不同资源类型使用不同的提示、结构校验和渲染方式，并通过状态机与进度事件反馈。

### 7.3 推送

推送机制先由多个查找器寻找候选，再由 AI 审核是否相关、必要和适合当前用户。系统建议与真实创建分离：先说明原因，用户确认后再生成或加入任务。

### 7.4 三层评估

1. **费曼式对话评估：** 用户能否用自己的话讲清机制。
2. **卡片准入审核：** 内容是否清晰、准确、必要，是否有反例、边界和关联。
3. **能力与学习健康评估：** 用户能否在陌生任务中迁移，长期表现是否稳定。

三层不能被一个总分抹平。卡片通过不等于能力掌握；当前高分也不等于长期保持。
...
RAG 不负责保存用户事实，不把所有聊天流水无差别写入索引，也不以相似度冒充事实关系。

### 8.2 知识图谱的职责

图谱由 Card、Edge、Cluster 等稳定对象组成。同一份事实数据可以投影为不同视图：
...
| 页面 | 单一职责 |
|---|---|
| Landing / 入口 | 解释产品并进入个人学习空间 |
| Dashboard | 展示当前课程知识库和学习状态总览 |
| Learn | 生成、执行和调整学习路径 |
| Forge | 围绕当前学习对象完成对话、输出、资源和审核 |
| Galaxy | 从多种认知视角查看知识结构 |
| Cognition | 查看画像、观察、证据和下一步建议 |

### 9.2 类型化 RPC

所有 API 调用通过 Hono RPC 客户端与自定义 Hook 完成。页面不直接散落 fetch 或 axios。服务端通过验证器、统一错误和领域服务保持薄网关。

### 9.3 状态管理

- React Query：服务端数据、失效和刷新。
- Zustand：跨页面 UI 模式与当前选择。
- 组件本地状态：仅用于局部交互。
- 领域状态：保存在服务器事实对象中，不由页面临时状态代替。
...
AXIOM Space 将概率生成结果置于可追溯、可校验、可复核的工程链路中：来源引用负责给出依据，结构校验负责约束输出形态，独立审核负责隔离生成与判定，工具确认负责控制写入动作，用户输出负责提供真实学习证据，审计记录负责还原关键过程。

该机制的工程目标是降低错误内容进入长期知识与掌握记录的概率，并为每项关键判断保留来源和复核入口。系统不以绝对化语言描述概率模型，而以证据完整性、行为边界和错误可恢复性建立可信度。

### 10.2 内容安全

外部资料、用户输入、模型输出和工具参数都需要边界检查。提示注入不得直接获得高风险工具权限，敏感数据不得写入日志、前端或导出。

### 10.3 性能和体验

- 流式输出尽早给出可见反馈。
- 资源状态区分排队、生成、校验、渲染、完成和失败。
- 多个资源可并行，但必须限制并发并隔离失败。
- 重资源可以先交付 HTML 或结构化预览，再异步完成最终媒体。
- 长任务不能用固定假进度掩盖真实等待。

### 10.4 适用边界与降级策略

- 系统运行状态以分项健康接口、执行日志和《04-测试说明书与测试报告》的复核记录为准。
- AI 评估输出掌握迹象、薄弱点与依据，并保留用户或教师复核入口。
- 外部模型、RAG 或渲染服务不可用时，系统采用明确错误、任务重试和派生能力降级；数据库事实与用户原始内容保持完整。
- 课程、画像和长期档案中的演示数据与真实用户研究数据分开管理，避免样例数据被解释为研究结论。
- 所有版本级能力认定统一回到需求编号、实现位置和测试证据，形成可审查的交付口径。

## 11. 需求到实现的追溯

| 需求 | 主要实现责任 |
|---|---|
| FR-001 | 认证、Vault 边界、API 鉴权、Prisma 归属 |
| FR-002 | 文档导入服务、卡片与关系、路径生成、索引任务 |
| FR-003—FR-004 | EducationProfile、Observation、画像协议、ContextBuilder |
| FR-005 | LearningPath、Step、PathAdjustment、路径服务 |
| FR-006—FR-008 | Agent Harness、双 Agent、工具契约、Forge、卡片审核 |
| FR-009—FR-011 | ResourceGenerationOrchestrator、状态机、推送查找器与确认 |
| FR-012 | RAG 服务、索引身份、引用回映射 |
| FR-013 | Card / Edge / Cluster、Galaxy 视图与交互 |
| FR-014—FR-016 | 教学策略、评估协议、Cognition 读模型 |
| FR-017—FR-018 | 导出、事件、通知、后台任务与审计 |

## 12. 对象真源与设计决策索引

附录 A、B 完整保留领域对象契约与开发技术规范，作为系统实现的工程真源。围绕产品问题、候选方案、取舍依据、交互规则和创新形成过程的 10 份设计记录，统一归入《10-产品创新与设计决策全记录》，从而使本说明书专注回答“系统如何设计并实现”，设计决策记录专注回答“为什么这样设计”。
...
![AXIOM Space DDD 领域边界与契约图](../01-SDD-文档驱动开发流程/assets/ddd-domain-boundary.png)
...
![AXIOM Space 开发调用边界图](../01-SDD-文档驱动开发流程/assets/dev-workflow-contract.png)
...
产品创新与方案取舍的完整证据见 [10-产品创新与设计决策全记录](./10-产品创新与设计决策全记录.md)。该文档逐篇保留 2 号目录的 10 份原始记录，并与本说明书建立如下对应关系：

| 设计决策记录 | 本说明书中的实现责任 |
|---|---|
| 01 技术架构说明 | 第 2、5、9、10 章：分层、事实源、集成与工程边界 |
| 02 自研垂直领域 Agent | 第 3、4、6、7 章：运行闭环、Harness、画像与评估 |
| 03 Web 工作台优先 | 第 9 章：页面职责、状态管理与系统集成 |
| 04 知识图谱创新 | 第 5、8 章：Card / Edge / Cluster 与认知投影 |
| 05 RAG 增强体验 | 第 5、8、10 章：事实/索引分离、召回、引用与降级 |
| 06 清晰、准确、必要 | 第 3、5、7、10 章：卡片准入、证据与可信边界 |
| 07 画像学习闭环 | 第 3、6、7 章：六维画像、策略编译与教学反馈 |
| 08 资源推送机制 | 第 7、9、10 章：候选发现、用户确认、状态与审计 |
| 09 根目录结构规则 | 第 9、10 章：工程组织、产物治理与可维护性 |
| 10 学习效果评估 | 第 3、6、7、10 章：费曼评估、卡片审核与反馈闭环 |

## 资料真源

- `docs/01-SDD-文档驱动开发流程/06-DDD-对象模型与契约.md`
- `docs/01-SDD-文档驱动开发流程/开发技术规范.md`
- [10-产品创新与设计决策全记录](./10-产品创新与设计决策全记录.md)
