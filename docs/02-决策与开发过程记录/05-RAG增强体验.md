...
这张图来自原始 LightRAG 架构文档的“真实链路”，重点是：保存卡片先落 Postgres，索引任务后台完成，页面通过状态看到结果。
...
正式演示时优先展示显式边，隐含边作为技术亮点说明，避免评委误以为所有边都是用户手写。
...
以下内容整合自非编号文档 `LightRAG 与后台知识增强架构.md` 和 `LightRAG 体验增强实施清单.md`，用于保留数据职责、当前链路、体验增强、技术亮点、接口和验收标准。
...
### 和 Hermes / Agent 架构的关系

LightRAG 不是替代 Hermes Agent，而是增强 Hermes Agent 的长期知识能力。

![Hermes / Postgres / Redis / LightRAG 协作图](./assets/decision-diagrams/05-hermes-rag-relation.png)
