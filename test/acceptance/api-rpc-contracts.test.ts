...
test('API/RPC contracts follow the 08 test plan for vault, graph, learning, events, and boundaries', async (t) => {
  await t.test('a fresh vault exposes an honest empty cognition profile', async () => {
    const { vault } = await createApiVault('empty-cognition')
    const response = await apiJson(`/api/cognition/stats?vid=${vault.id}`)

    assert.equal(response.status, 200)
    assert.equal(response.body.success, true)
    assert.deepEqual(response.body.dimensionInsights, [])
    assert.equal(response.body.promptBlock, '')
    assert.deepEqual(response.body.interventionRuns, [])

    const prompt = await apiJson(`/api/cognition/summarize-prompt?vid=${vault.id}`, { method: 'POST' })
    assert.equal(prompt.status, 409)
    assert.equal(prompt.body.error, 'PROFILE_EVIDENCE_REQUIRED')
  ...
  await t.test('vault write/read/update/link/delete round trip preserves ownership and link state', async () => {
    const { user, vault } = await createApiVault('vault-roundtrip')
    ...
    await createPassingPromotionEvidence(user.id, vault.id, source.id, 'Source Updated')
  ...
  await t.test('promoting a card to permanent archives only its bound card thread', async () => {
    ...
    await createPassingPromotionEvidence(user.id, vault.id, promotedCard.id, promotedCard.title ?? 'Promotion Source')
...
## 边界与依据
它不是一段可以直接复制的标准答案；必须依据学习者自己的解释和评估结果来判断。因为缺少这条证据链会让永久卡失去可验证性，所以这部分不可省略。

## 应用
在实践场景中，${title} 可以用于整理知识、指导决策，并帮助后续学习路径落地。`
}

async function createPassingPromotionEvidence(userId: string, vaultId: string, cardId: string, concept: string) {
  return prisma.assessmentResult.create({
    data: {
      ...
      passed: true,
      mastery: 85,
      feedback: '测试夹具：已通过独立评估。',
      evidence: JSON.stringify(['test:promotion-evidence']),
      clientContext: JSON.stringify({ rubricId: 'concept-transfer-v1', deterministicCheck: 'not_required' }),
    },
  })
}
